...
function updateWantToConfirm(want_item_id, required_item_id, con) {
  return new Promise((resolve, reject) => {
    let queryString = 'UPDATE want SET confirmed = 1 WHERE want_item_id = ? AND required_item_id = ?';
...
async function checkDoubleMatch(curUserItemId, required_item_id, con) {
  // 檢查 double 是否成功配對 :
  return new Promise((resolve, reject) => {
    let queryString = 'SELECT w.required_item_id, w.want_item_id FROM want w WHERE w.want_item_id = ? AND w.required_item_id = ? AND w.confirmed = 1';
