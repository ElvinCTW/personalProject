...
const {addNewMatchedPageMsg,
  getConfirmedMatchMsg,
  getLastestMsg,
  getInsertedMsgTime} = require('../model/msg');
const {getConfirmedMatches} = require('../model/item');
const {getConfirmedMatchItemsData} = require('../model/item');
const {getConfirmedMatchItemsId} = require('../model/match');
const {checkVaildUserOfChat} = require('../model/user');
...
function init(server) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join', async (obj) => {
      const confirmedMatchArr = await getConfirmedMatches(obj.token)
          .catch(() => {
      ...
      const matchedIdArr =
      confirmedMatchArr.map((match) => `${match.matched_id}`);
      socket.join(matchedIdArr); // 把 socket 加入所屬交易的對話房間(s)
      // 取得每個 match 的最新一筆訊息
      const lastestMsgArr = await getLastestMsg(matchedIdArr).catch(() => { });
    ...
    socket.on('history', async (obj) => {
      ...
      await checkVaildUserOfChat(obj.token, obj.matched_id)
          .catch(() => {
            const errorMsg = '暫時無法認證您的身份QQ，若持續發生請聯絡我們';
            io.to(socket.id).emit('error', {errorMsg});
          });
      if (checkValidUser) {
        // content, sender
        const msgArr = await getConfirmedMatchMsg(obj.matched_id)
            .catch(() => {
        ...
        if (msgArr) {
          // 取得 matched_items_id
          const matchedItemsIdObj =
          await getConfirmedMatchItemsId(obj.matched_id)
              .catch(() => {
                const errorMsg = '暫時無法取得通訊紀錄QQ，若持續發生請聯絡我們';
                io.to(socket.id).emit('error', {errorMsg});
              });
          if (matchedItemsIdObj) {
            // 整理取得 item Data
            const idArr = Object.values(matchedItemsIdObj);
            const itemDataArr = await getConfirmedMatchItemsData(idArr)
                .catch(() => { });
            if (itemDataArr) {
    ...
    socket.on('message', async (obj) => {
      // 確認本人
      const checkUser = await checkVaildUserOfChat(obj.token, obj.matched_id)
          .catch(() => {
