  ...
  get: (queryCondition) => {
    return new Promise((resolve, reject) => {
      ...
      if (queryCondition.action === 'getConfirmedMatchItemsData') {
        ...
        console.log('connection,itemDAO')
        console.log(queryCondition.connection)
        // item detail info page
        queryString += 'WHERE id = ? AND availability = "true"'
        queryCondition.connection.query(queryString, queryCondition.item_id, (err, getItemResultArr, fields) => {
          if (err) { 
            queryCondition.connection.rollback(()=>{
              queryCondition.connection.release()
            })
            reject(err) 
          } else {
            queryCondition.connection.commit(function (err) {
              if (err) {
                queryCondition.connection.rollback(function () {
                  queryCondition.connection.release();
                  //Failure
                });
              } else {
